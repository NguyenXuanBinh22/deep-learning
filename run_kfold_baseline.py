...
(Baseline version không có contrastive learning)
"""
import os
import sys
import numpy as np
...
# Import training function từ moBRCA-net_baseline (file có dấu gạch ngang nên dùng importlib)
# Tìm đường dẫn đến moBRCA-net_baseline.py dựa trên thư mục của script này
script_dir = os.path.dirname(os.path.abspath(__file__))
mobrca_net_baseline_path = os.path.join(script_dir, "moBRCA-net_baseline.py")
spec = importlib.util.spec_from_file_location("mobrca_net_baseline", mobrca_net_baseline_path)
mobrca_net_baseline = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mobrca_net_baseline)
train_and_eval = mobrca_net_baseline.train_and_eval
MultiOmicsDataset = mobrca_net_baseline.MultiOmicsDataset
...
def run_kfold_cv(args):
    """
    Chạy k-fold cross-validation cho baseline model.
    """
    ...
    print("=" * 80)
    print("STRATIFIED K-FOLD CROSS-VALIDATION (BASELINE)")
    print("=" * 80)
    print(f"Model: moBRCA-net BASELINE (no contrastive learning)")
    ...
    results_dir = os.path.join(base_dir, "kfold_results_baseline")
    ...
    # Tổng hợp kết quả
    print(f"\n{'='*80}")
    print("K-FOLD CROSS-VALIDATION SUMMARY (BASELINE)")
    print(f"{'='*80}\n")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run k-fold cross-validation for moBRCA-net BASELINE (no contrastive learning)"
    )
